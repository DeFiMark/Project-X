...
interface IFlashBorrower {
     ...
     * @param token The loan currency, must be Staking Token
...
contract MAXIStaking is Ownable, IERC20, IFlashLender {
    ...
    function maxFlashLoan(address _token) public view override returns (uint256) {
        return _token == address(token) ? token.balanceOf(address(this)) : 0;
    }
    ...
    /**
     * @dev Initiate a flash loan, borrowing Token
     * @param receiver The receiver of the tokens in the loan, and the receiver of the callback.
     * @param amount The amount of tokens lent.
     * @param data Arbitrary data structure, intended to contain user-defined parameters.
     */
    function flashLoan(
        ...
        bytes calldata data
         ...
         require(
            amount > 0 &&
            amount <= maxFlashLoan(address(token)),         
        ...
        // require more Token was returned
        require(
            token.balanceOf(address(this)) >= amountBefore + fee,
        ...
        // get new price and validate it rose
